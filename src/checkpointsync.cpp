...
// cmc: sync-checkpoint master key
...
// cpuminerscoin: synchronized checkpoint (centrally broadcasted)
...
// cpuminerscoin: get last synchronized checkpoint
...
// cpuminerscoin: only descendant of current sync-checkpoint is allowed
...
bool CheckSyncCheckpoint(const uint256& hashBlock, const CBlockIndex* pindexPrev)
{
   ...
   if (pindexPrev->nHeight > 0 && chainActive.Height() == 0) return true;
    ...
    //cmcTODO - needs rewrite, because it is very slow during accepting headers
    return true;
...
// cpuminerscoin: reset synchronized checkpoint to last hardened checkpoint
...
// cpuminerscoin: verify signature of sync-checkpoint message
...
// cpuminerscoin: process synchronized checkpoint
