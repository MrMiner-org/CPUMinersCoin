...
// cpuminerscoin: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr <CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, bool fMineWitnessTx, CWallet *pwallet, bool *pfPoSCancel) {
    ...
    // cpuminerscoin: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // cpuminerscoin: there is no point to continue if we failed to create coinstake
    }
...
//        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CMCMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet) {
    ...
    RenameThread("cpuminerscoin-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            // cpuminerscoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake()) {
...
// cpuminerscoin: stake minter thread
...
// cpuminerscoin: stake minter
void MintStake(boost::thread_group &threadGroup) {
    // cpuminerscoin: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
...
bool static ScanHash(const CBlockHeader *pblock, uint32_t &nNonce, uint256 *phash) {
    while (true) {
        ...
        // Calculate CMC for given block and nonce
        CMC(pblock, nNonce, phash);
...
void static CMCMiner(const CChainParams &chainparams, void *parg) {
    LogPrintf("CMCMiner started\n");
    RenameThread("cpuminerscoin-miner");
    CWallet *pwallet = (CWallet *) parg;
    ...
    try {
        ...
        while (true) {
          ...
          //  }
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in CMCMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running CMCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        LOCK2(cs_main, pwallet->cs_wallet);
                        if (!SignBlock(*pblock, *pwallet)) {
                            LogPrintf("CMCMiner(): failed to sign PoS block");
                            continue;
                        }
        ...
        LogPrintf("CMCMiner terminated!\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("CMCMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateCMC(bool fGenerate, int nThreads, const CChainParams &chainparams) {
    ...
    if (!vpwallets.empty()) {
        for (int i = 0; i < nThreads; i++)
            minerThreads->create_thread(boost::bind(&CMCMiner, boost::cref(chainparams), vpwallets[0]));
    }
}
