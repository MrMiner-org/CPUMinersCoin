...
#if defined(NDEBUG)
# error "CMC cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "CMC Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // cpuminerscoin: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
// cpuminerscoin: it depends on current POW Block Height, not current blockchain Height
int64_t GetProofOfWorkReward(unsigned int nHeight) {
    ...
    else if (nHeight > 1 && nHeight <= 20000000) {
    return 1 * COIN;
    }
    else {
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // cpuminerscoin: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // cpuminerscoin
            undo.nTime = alternate.nTime;           // cpuminerscoin
        } else {
...
void ThreadScriptCheck() {
    RenameThread("cpuminerscoin-scriptch");
...
bool CMCContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // cpuminerscoin: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!CMCContextualBlockChecks(block, state, pindex, fJustCheck))
  ...
  //  if (block.IsProofOfWork() && (pindex->nHeight > 0 && pindex->pprev->nPOWBlockHeight + 1 > params.nTotalPOWBlock))
    ...
    // cpuminerscoin: coinbase reward check relocated to CheckBlock()
    ...
    // cpuminerscoin: track money supply and mint amount info
    ...
    // cpuminerscoin: fees are not collected by miners as in bitcoin
    // cpuminerscoin: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // cpuminerscoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // cpuminerscoin: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // cpuminerscoin: check transaction timestamp
    ...
    // cpuminerscoin: check block signature
...
#ifdef ENABLE_CHECKPOINTS
    // cpuminerscoin: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // cpuminerscoin: Don't reject in case of old clients. Change our assumption instead.
        //cmcTODO: Maybe add restrictions until when this is allowed? We don't want new clients to pretend to be old clients and try to abuse this.
    ...
    //cmcTODO - move this somewhere in the upper calls, where pfrom is visible
//    // cpuminerscoin: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // cpuminerscoin: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // cpuminerscoin: check PoS
    if (fCheckPoS && !CMCContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // cpuminerscoin: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // cpuminerscoin: if responsible for sync-checkpoint send it
...
#ifdef ENABLE_CHECKPOINTS
    // cpuminerscoin: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // cpuminerscoin: txindex is always enabled
...
// cpuminerscoin: total coin age spent in transaction, in the unit of coin-days.
