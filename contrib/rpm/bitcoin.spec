...
install -p %{SOURCE100} %{buildroot}%{_datadir}/pixmaps/cpuminerscoin.png
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15996
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15995
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15997
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15994
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15998
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15993
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 15996
	%{_sbindir}/semanage port -d -p tcp 15995
	%{_sbindir}/semanage port -d -p tcp 15997
	%{_sbindir}/semanage port -d -p tcp 15994
	%{_sbindir}/semanage port -d -p tcp 15998
	%{_sbindir}/semanage port -d -p tcp 15993
	for selinuxvariant in %{selinux_variants}; do
